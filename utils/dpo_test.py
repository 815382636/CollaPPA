...
os.environ["WANDB_API_KEY"] = '016d58c00d77c7c69951b3c4c690bd278ae2716e' # 将引号内的+替换成自己在wandb上的一串值
...
wandb.init(project="DPO4rec", config=vars(args))
...
def train():
    ...
    while True:
        ...
        with torch.no_grad():    
            ...
            for i in range(task.size(0)):
                ...
                for index,j in enumerate(results):
                    ...
                    if index == len(results) - 1 and len(formatted_completions) <= i:
        ...
        with torch.no_grad():  
            ref_log_probs_lose = compute_log_probs(model=ref_model, generate=target_seq, encoder_tensor=encoder_tensor)
            ref_log_probs_win = compute_log_probs(model=ref_model, generate=target, encoder_tensor=encoder_tensor)      

        log_probs_lose = compute_log_probs(model=model, generate=target_seq, encoder_tensor=encoder_tensor)
